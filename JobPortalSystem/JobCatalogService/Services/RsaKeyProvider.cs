...
/// <summary>
/// Lazily fetches and caches the RSA public key from IdentityService.
/// GetKey() is synchronous so it can be used in IssuerSigningKeyResolver.
/// </summary>
public class RsaKeyProvider(IConfiguration config, ILogger<RsaKeyProvider> logger)
{
    ...
    public RsaSecurityKey? GetKey()
    {
        ...
        lock (_lock)
        {
            ...
            {
                logger.LogWarning(ex, "Could not fetch RSA public key from {Url}", _publicKeyUrl);
            }
        }
        return _cachedKey;
    }
